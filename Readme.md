...
> **V6.0 新版本**：规则库大规模扩充至 16,858 条，重写置信度引擎，新增 Server 版本提取、默认口令标注、规则质量审计等功能。详见 [V6.0 新增功能](#v60-新增功能)。
...
- **规则库扩充**：合并 17 个指纹库来源，254K 原始规则去重清洗 → **16,858 条规则，覆盖 11,945 产品**
...
- **默认口令标注**：xlsx 新增 `DefaultCreds` 列，覆盖 34 个常见产品
...
| ISP | 运营商（需 --geo） |
| DefaultCreds | 🆕 已知默认口令（如 `nacos/nacos`, `admin/admin`） |
