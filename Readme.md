...
- **指纹识别** — 16347 条规则，覆盖 CMS、OA、框架、防火墙、路由器、摄像头、CDN 等
...
- **指纹识别**：keyword 规则以导入时质量过滤为主，运行时不降权。短关键词不一定是低质量（如 AKCMS 是专有产品名）。建议扫描后人工复核结果。
